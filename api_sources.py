...
class APISourceManager:
    ...
    2. Собирает НП из общих списков, страниц бывших НП и сельских поселений
    3. Для НП без координат:
       - Сначала ищет на dic.academic.ru
    ...
    def __init__(self):
        ...
        # Статистика
        self.stats = {
            'from_former': 0, 'from_dic_link': 0, 'from_wiki': 0,
            'from_cache': 0, 'total_without': 0, 'remaining': 0
        }
    ...
    def clear_cache(self):
        ...
        self.stats = {'from_former': 0, 'from_dic_link': 0, 'from_wiki': 0,
                      'from_cache': 0, 'total_without': 0, 'remaining': 0}
    ...
    async def _rate_limit(self):
        ...
        if self.request_count % 50 == 0:
            logger.info(f"    Запросов: {self.request_count} за {time.time() - self.start_time:.1f}с")
        ...
        for r in sorted_results[:5]:
            if r['score'] >= 50:
                ...
                if html and self._is_district_page(html, district):
                    ...
                    logger.info(f"    ✅ Найдена страница района (ID: {r['id']})")
                    return info
    ...
    async def _find_former_page(self, settlement: str, district: str) -> Optional[str]:
        ...
        if best['score'] >= 50:
            logger.info(f"      Найдена страница бывших НП для {settlement} (ID: {best['id']})")
    ...
    async def _find_settlement_page(self, settlement: str, district: str) -> Optional[str]:
        ...
        if best['score'] >= 40:
            logger.info(f"      Найдена страница СП {settlement} (ID: {best['id']})")
    ...
    async def _parse_former_page(self, article_id: str, district: str, settlement: str) -> List[Dict]:
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(self.thread_pool, self._parse_former_html, html, district)
    
    def _parse_former_html(self, html: str, district: str) -> List[Dict]:
        try:
            soup = BeautifulSoup(html, 'html.parser')
            results = []
            ...
            for table in soup.find_all('table', class_=['standard', 'sortable']):
                ...
                for row in rows[1:]:
                    ...
                    if len(cells) <= max(name_idx or 0, type_idx or 0):
                        continue
    ...
    async def _parse_settlement_page(self, article_id: str, district: str, settlement: str) -> List[Dict]:
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(self.thread_pool, self._parse_settlement_html, html, district)
    
    def _parse_settlement_html(self, html: str, district: str) -> List[Dict]:
        try:
            soup = BeautifulSoup(html, 'html.parser')
            results = []
    ...
    async def fetch_district_data(self, district: str) -> List[Dict]:
                ...
                if any(kw in text for kw in LIST_KEYWORDS):
                    ...
                    if match:
                        ...
                        if list_id not in processed_lists:
                            ...
                            logger.info(f"  🔍 Обрабатываем список ID {list_id}")
                        ...
                        if key not in seen:
                            ...
                            for i, existing in enumerate(all_villages):
                                if f"{existing['name']}_{existing['district']}" == key:
                                    if not existing.get('has_coords') and v.get('has_coords'):
                                        all_villages[i] = v
        ...
        self.stats['remaining'] = len(without_coords) - found - self.stats['from_former']
        
        logger.info(f"    ✅ Поиск завершен. Найдено: {found}")
        logger.info(f"    📊 ИТОГО ПО КООРДИНАТАМ:")
        logger.info(f"      • Из бывших НП: {self.stats['from_former']}")
        logger.info(f"      • Из кэша: {self.stats['from_cache']}")
        logger.info(f"      • Из dic.academic.ru: {self.stats['from_dic_link']}")
        logger.info(f"      • Из Wikipedia: {self.stats['from_wiki']}")
        logger.info(f"      • Всего: {self.stats['from_former'] + self.stats['from_cache'] + self.stats['from_dic_link'] + self.stats['from_wiki']}")
        logger.info(f"      • Осталось: {self.stats['remaining']}")
        
        # Сортируем и возвращаем
        all_villages.sort(key=lambda x: x['name'])
        
        total_time = time.time() - self.start_time
        final_coords = sum(1 for v in all_villages if v.get('has_coords'))
        logger.info(f"  ✅ Всего: {len(all_villages)} НП")
        logger.info(f"  ✅ С координатами: {final_coords}")
        logger.info(f"  ⏱️ Время: {total_time:.1f}с")
