...
class APISourceManager:
    ...
    async def _find_coordinates_for_village(self, village: Dict, district: str) -> Optional[Dict]:
        """
        Оптимизированный поиск координат: сначала ссылки, потом поиск
        """
        try:
            # Вариант 1: Используем сохраненную ссылку (БЕЗ ПОИСКА)
            ...
            # Вариант 2: Точный поиск с кавычками и типом
            type_short = TYPE_SHORT.get(village['type'], '')
            if type_short:
                exact_query = f'"{village["name"]}" {type_short} {district} район'
            else:
            ...
            # Вариант 3: Поиск с типом и районом (без кавычек)
            if type_short:
                simple_query = f"{village['name']} {type_short} {district} район"
            else:
            ...
            # Вариант 4: Поиск по названию и области
            region_query = f"{village['name']} Тверская область"
            results = await self._search_with_pagination(region_query, max_pages=3, unlimited=False)
    ...
    async def fetch_district_data(self, district: str) -> List[Dict]:
        ...
        # Шаг 5: Поиск координат для всех записей без них
        if all_villages:
            ...
            if villages_without_coords:
                ...
                # Сортируем: сначала те, у кого есть ссылки
                ...
                # Дальше по приоритету: сначала деревни/села/посёлки
                ...
                # Исправленный подсчет остатка (не вычитаем from_former, так как это уже было в исходных данных)
                self.coords_stats['remaining'] = total_without - self.coords_stats['from_links'] - search_found
                ...
                logger.info(f"      • Всего найдено: {self.coords_stats['from_links'] + search_found}")
