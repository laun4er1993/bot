# api_sources.py
# Универсальный парсер для всех районов через dic.academic.ru и Wikipedia
# Оптимизированная версия: сначала Wikipedia (прямые ссылки), потом dic.academic.ru
...
# Соответствие районов возможным названиям на Wikipedia
DISTRICT_WIKI_NAMES = {
...
# Ключевые слова для поиска ссылок на списки НП (dic.academic.ru) - оставляем для обратной совместимости
LIST_KEYWORDS = [
...
class APISourceManager:
    ...
    def __init__(self):
        ...
        # Кэш для найденных ID (dic.academic.ru) - для fallback
        ...
        # Словарь для хранения ссылок на отдельные страницы НП (с dic.academic.ru) - для fallback
        self.village_links: Dict[str, str] = {}  # название НП -> ID статьи
        
        # Словарь для хранения Wikipedia ссылок на НП (ОСНОВНОЙ ИСТОЧНИК)
        self.wikipedia_village_links: Dict[str, str] = {}  # название НП -> URL статьи
        
        # ВРЕМЕННЫЙ КЭШ КООРДИНАТ (ТОЛЬКО ДЛЯ ТЕКУЩЕГО ПОИСКА)
        self.temp_coords_cache: Dict[str, Tuple[str, str]] = {}  # название -> (lat, lon)
        ...
        self.coords_stats = {
            'from_former': 0,
            'from_links_dic': 0,      # из dic.academic.ru по ссылке
            'from_search_dic': 0,      # из поиска на dic.academic.ru
            'from_temp_cache': 0,
            'from_wikipedia': 0,       # из Wikipedia (ОСНОВНОЙ)
            'total_without': 0,
            'remaining': 0
        }
    ...
    def clear_cache(self):
        ...
        self.wikipedia_village_links.clear()
        self.temp_coords_cache.clear()
        self.coords_stats = {
            'from_former': 0,
            'from_links_dic': 0,
            'from_search_dic': 0,
        ...
        logger.info("🧹 Все кэши очищены для нового поиска")
    ...
    async def _fetch_page_with_retry(self, url: str, retry_count: int = 0) -> Optional[str]:
        ...
        if retry_count >= self.max_retries:
            logger.debug(f"Превышено количество попыток для {url}")
            return None
    ...
    # ========== МЕТОДЫ ДЛЯ РАБОТЫ С DIC.ACADEMIC.RU (FALLBACK) ==========
    
    async def _search_with_pagination(self, query: str, max_pages: int = 10, unlimited: bool = False) -> List[Dict]:
        """Выполняет поиск с обработкой нескольких страниц результатов (для dic.academic.ru)"""
        all_results = []
        page = 1
    ...
    def _parse_search_page(self, html: str, page_num: int) -> List[Dict]:
        """Парсит одну страницу результатов поиска dic.academic.ru"""
        try:
    ...
    # ========== МЕТОДЫ ДЛЯ РАБОТЫ С DIC.ACADEMIC.RU (СБОР НП) ==========
    # Эти методы оставляем без изменений - они собирают НП из разных источников
    ...
    async def _find_district_page(self, district: str) -> Optional[Dict]:
        ...
        for result in top_results:
            if result['score'] >= 50:
                ...
                if html:
                    ...
                    if is_district:
                        logger.info(f"    ✅ Найдена страница района на dic.academic.ru (ID: {result['id']}, score: {result['score']})")
        ...
        logger.info(f"    ❌ Страница района не найдена на dic.academic.ru")
        return None
    ...
    async def _extract_settlements_from_page(self, html: str, district: str) -> List[str]:
        """Извлекает список сельских поселений со страницы района dic.academic.ru"""
        try:
            soup = BeautifulSoup(html, 'html.parser')
            found_settlements = []
    ...
    async def _find_settlement_main_page(self, settlement: str, district: str) -> Optional[str]:
        """Находит основную страницу сельского поселения на dic.academic.ru"""
    ...
    async def _parse_former_np_page(self, article_id: str, district: str, settlement: str) -> List[Dict]:
        """Парсит страницу с бывшими населенными пунктами на dic.academic.ru"""
            ...
            for table in tables:
                ...
                for row in rows[1:]:
                    try:
                        ...
                        if lat and lon:
                            self.temp_coords_cache[name] = (str(round(lat, 5)), str(round(lon, 5)))
    ...
    async def _parse_settlement_main_page(self, article_id: str, district: str, settlement: str) -> List[Dict]:
        """Парсит основную страницу сельского поселения на dic.academic.ru"""
        url = DIC_ACADEMIC_ARTICLE_URL.format(article_id)
        html = await self._fetch_page(url)
    ...
    async def _find_master_list_links(self, html: str, district: str) -> List[str]:
        """Автоматический поиск ссылок на списки населенных пунктов на dic.academic.ru"""
        try:
            soup = BeautifulSoup(html, 'html.parser')
            found_ids = []
    ...
    async def _parse_master_list_page(self, article_id: str, district: str) -> List[Dict]:
        """Парсит страницу со списком населенных пунктов на dic.academic.ru"""
        url = DIC_ACADEMIC_ARTICLE_URL.format(article_id)
        html = await self._fetch_page(url)
    ...
    async def _parse_individual_village_page_dic(self, article_id: str, district: str) -> Optional[Dict]:
        """Парсит отдельную страницу населенного пункта для извлечения координат (dic.academic.ru) - FALLBACK"""
        url = DIC_ACADEMIC_ARTICLE_URL.format(article_id)
        html = await self._fetch_page(url)
        ...
        return await loop.run_in_executor(
            self.thread_pool,
            self._parse_individual_village_html_dic,
            html,
            article_id,
            district
        )
    
    def _parse_individual_village_html_dic(self, html: str, article_id: str, district: str) -> Optional[Dict]:
        """Парсит отдельную страницу населенного пункта (dic.academic.ru) - FALLBACK"""
        try:
            soup = BeautifulSoup(html, 'html.parser')
    ...
    # ========== НОВЫЕ МЕТОДЫ ДЛЯ РАБОТЫ С WIKIPEDIA (ОСНОВНОЙ ИСТОЧНИК) ==========
    ...
    async def _find_wikipedia_district_page(self, district: str) -> Optional[str]:
        ...
        # Пробуем возможные названия района на Wikipedia
        possible_names = DISTRICT_WIKI_NAMES.get(district, [district, f"{district} район"])
        ...
        for name in possible_names:
            ...
            if html:
                ...
                if not no_article_div:
                    # Проверяем, что это страница района
                    text = soup.get_text().lower()
                    if ('муниципальный округ' in text or 'муниципальный район' in text) and 'тверская область' in text:
        ...
        search_url = f"{WIKIPEDIA_SEARCH_URL}?action=query&list=search&srsearch={quote_plus(f'{district} район Тверская область')}&format=json&utf8=1"
        ...
        if html:
            try:
                ...
                logger.debug(f"Ошибка парсинга Wikipedia API: {e}")
        
        logger.warning(f"    ❌ Страница района на Wikipedia не найдена")
        return None
    
    async def _extract_wikipedia_village_links(self, district_page_url: str, district: str) -> Dict[str, str]:
        """
        Извлекает из страницы района на Wikipedia ссылки на статьи населенных пунктов
        Возвращает словарь {название_НП: URL_статьи}
        """
        logger.info(f"  🔍 Извлечение ссылок на НП из Wikipedia страницы района")
        ...
        for table in tables:
            # Проверяем, есть ли в таблице заголовок "Населённый пункт"
            headers = table.find_all('th')
            has_village_header = False
            village_col_index = None
            
            for i, header in enumerate(headers):
                header_text = header.get_text().strip().lower()
                if 'населённый пункт' in header_text or 'населенный пункт' in header_text or 'название' in header_text:
                    has_village_header = True
                    village_col_index = i
                    break
            
            if not has_village_header:
                continue
            ...
            for row in rows[1:]:  # Пропускаем заголовок
                cells = row.find_all('td')
                if not cells or len(cells) <= village_col_index:
                    continue
                
                village_cell = cells[village_col_index]
                link = village_cell.find('a')
                
                if link and link.get('href'):
                    href = link.get('href')
                    if href.startswith('/wiki/') and ':' not in href:
                        name = link.get_text().strip()
                        # Очищаем название от сносок [1], [2] и т.д.
                        name = re.sub(r'\[\d+\]', '', name).strip()
                        
                        if name and self._is_valid_name(name):
                            full_url = f"{WIKIPEDIA_BASE_URL}{href}"
                            village_links[name] = full_url
                            logger.debug(f"      Найдена ссылка: {name} -> {full_url}")
        
        logger.info(f"    Найдено ссылок на Wikipedia: {len(village_links)}")
        return village_links
    
    async def _parse_wikipedia_coordinates(self, html: str) -> Optional[Tuple[str, str]]:
        """
    ...
    async def _get_wikipedia_coordinates_for_village(self, village_name: str, wiki_url: str) -> Optional[Tuple[str, str]]:
        """
        Загружает страницу НП на Wikipedia и извлекает координаты
        """
        try:
            logger.info(f"        🔍 Wikipedia: загрузка {wiki_url}")
    ...
    # ========== ОСНОВНОЙ МЕТОД ПОИСКА КООРДИНАТ (ОПТИМИЗИРОВАННЫЙ) ==========
    
    async def _find_coordinates_for_village(self, village: Dict, district: str, 
                                             wikipedia_links: Dict[str, str] = None) -> Optional[Dict]:
        """
        ОПТИМИЗИРОВАННЫЙ поиск координат:
        1. Временный кэш
        2. Wikipedia (прямая ссылка) - ОСНОВНОЙ ИСТОЧНИК
        3. dic.academic.ru (только если нет ссылки в Wikipedia) - FALLBACK
        """
        try:
            village_name = village['name']
            
            # ВАРИАНТ 0: Проверяем временный кэш
            if village_name in self.temp_coords_cache:
                lat, lon = self.temp_coords_cache[village_name]
                logger.info(f"        📍 ИЗ ВРЕМЕННОГО КЭША: {village_name}: {lat}, {lon}")
                self.coords_stats['from_temp_cache'] += 1
                return {
                    "name": village_name,
                    "type": village['type'],
                    "lat": lat,
            ...
            # ВАРИАНТ 1: Wikipedia - ОСНОВНОЙ ИСТОЧНИК (самый быстрый и точный)
            if wikipedia_links and village_name in wikipedia_links:
                wiki_url = wikipedia_links[village_name]
                logger.info(f"        🔍 Поиск по ссылке Wikipedia для {village_name}")
                coords = await self._get_wikipedia_coordinates_for_village(village_name, wiki_url)
                if coords:
                    lat, lon = coords
            ...
            # ВАРИАНТ 2: dic.academic.ru - FALLBACK (только если нет в Wikipedia)
            if village_name in self.village_links:
                ...
                if article_id not in self.processed_article_ids:
                    ...
                    logger.info(f"        🔍 Поиск по ссылке dic.academic.ru для {village_name} (FALLBACK)")
                    village_data = await self._parse_individual_village_page_dic(article_id, district)
                    if village_data and village_data.get('has_coords'):
                        self.coords_stats['from_links_dic'] += 1
                        return village_data
            
            return None
            
        except Exception as e:
            logger.error(f"Ошибка поиска координат для {village['name']}: {e}")
            return None
    ...
    async def fetch_district_data(self, district: str) -> List[Dict]:
        ...
        # Шаг 1: Находим страницу района на dic.academic.ru (для сбора НП)
        ...
        # Шаг 3: Ищем общие списки на dic.academic.ru
        if district_html:
        ...
        # Шаг 4: Для каждого СП ищем страницы на dic.academic.ru (для сбора НП)
        ...
        # ========== ОСНОВНОЙ ЭТАП: ПОИСК КООРДИНАТ ЧЕРЕЗ WIKIPEDIA ==========
        
        # Шаг 5: Находим страницу района на Wikipedia
        ...
        if wikipedia_page_url:
            ...
            logger.info(f"  📊 Найдено {len(wikipedia_links)} ссылок на НП в Wikipedia")
        else:
            logger.warning(f"  ⚠️ Страница района на Wikipedia не найдена")
        
        # Шаг 6: Поиск координат для записей без них (ТОЛЬКО Wikipedia, потом FALLBACK)
        if all_villages:
            ...
            logger.info(f"    • Всего записей: {len(all_villages)}")
            ...
            if villages_without_coords:
                logger.info(f"    • Ссылок на Wikipedia: {len(wikipedia_links)}")
                logger.info(f"    • Ссылок на dic.academic.ru (FALLBACK): {len(self.village_links)}")
                logger.info(f"    • Временный кэш координат: {len(self.temp_coords_cache)}")
                
                # Сортируем по приоритету: сначала те, у кого есть ссылки на Wikipedia
                ...
                priority_fallback = [v for v in without_wiki_links if v['type'] in ['деревня', 'село', 'посёлок']]
                other_fallback = [v for v in without_wiki_links if v['type'] not in ['деревня', 'село', 'посёлок']]
                
                sorted_villages = priority_wiki + other_wiki + priority_fallback + other_fallback
                
                logger.info(f"    • С приоритетом по ссылкам Wikipedia: {len(with_wiki_links)}")
                logger.info(f"    • Приоритетных записей (деревни/села/посёлки): {len(priority_wiki) + len(priority_fallback)}")
                ...
                for i, village in enumerate(sorted_villages):
                    try:
                        ...
                        village_data = await self._find_coordinates_for_village(village, district, wikipedia_links)
                        
                        if village_data and village_data.get('has_coords'):
                            for v in all_villages:
                                if v['name'] == village['name']:
                                    v['lat'] = village_data['lat']
                                    v['lon'] = village_data['lon']
                                    v['has_coords'] = True
                                    search_found += 1
                                    logger.info(f"        ✅ ДОБАВЛЕНЫ КООРДИНАТЫ В БАЗУ: {village['name']} -> {village_data['lat']}, {village_data['lon']}")
                                    break
                        ...
                        logger.error(f"      Ошибка обработки {village.get('name')}: {e}")
                        continue
                
                self.coords_stats['from_search_dic'] = search_found
                self.coords_stats['remaining'] = total_without - self.coords_stats['from_former'] - self.coords_stats['from_links_dic'] - self.coords_stats['from_temp_cache'] - self.coords_stats['from_wikipedia'] - self.coords_stats['from_search_dic']
                ...
                logger.info(f"      • Из списка бывших НП: {self.coords_stats['from_former']}")
                logger.info(f"      • Из временного кэша: {self.coords_stats['from_temp_cache']}")
                logger.info(f"      • Найдено через Wikipedia (ОСНОВНОЙ): {self.coords_stats['from_wikipedia']}")
                logger.info(f"      • Найдено через dic.academic.ru (FALLBACK): {self.coords_stats['from_links_dic'] + self.coords_stats['from_search_dic']}")
                logger.info(f"      • Всего найдено: {self.coords_stats['from_former'] + self.coords_stats['from_temp_cache'] + self.coords_stats['from_wikipedia'] + self.coords_stats['from_links_dic'] + self.coords_stats['from_search_dic']}")
