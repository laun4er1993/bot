...
class APISourceManager:
    ...
    def __init__(self):
        ...
        # КЭШ ДЛЯ КООРДИНАТ - НОВОЕ!
        self.coordinates_cache: Dict[str, Tuple[str, str]] = {}  # название -> (lat, lon)
    ...
    def clear_cache(self):
        ...
        # НЕ ОЧИЩАЕМ coordinates_cache! Он должен сохраняться между запусками
        self.coords_stats = {
            'from_former': 0,
            'from_links': 0,
            'from_search': 0,
        ...
        logger.info("🧹 Кэш очищен для нового поиска (координаты сохранены)")
    ...
    async def _search_with_pagination(self, query: str, max_pages: int = 10, unlimited: bool = False) -> List[Dict]:
        ...
        ОПТИМИЗИРОВАНО: уменьшено max_pages с 30 до 10
        """
        all_results = []
        page = 1
            ...
            for table in tables:
                ...
                for row in rows[1:]:
                    try:
                        ...
                        if lat and lon:
                            # СОХРАНЯЕМ В КЭШ КООРДИНАТ!
                            self.coordinates_cache[name] = (str(round(lat, 5)), str(round(lon, 5)))
    ...
    async def _find_coordinates_for_village(self, village: Dict, district: str) -> Optional[Dict]:
        """
        ОПТИМИЗИРОВАННЫЙ поиск координат: сначала кэш, потом ссылки, потом поиск
        """
        try:
            village_name = village['name']
            
            # ВАРИАНТ 0: Проверяем кэш координат (САМЫЙ БЫСТРЫЙ!)
            if village_name in self.coordinates_cache:
                lat, lon = self.coordinates_cache[village_name]
                logger.info(f"        📍 КЭШ: координаты для {village_name} из кэша: {lat}, {lon}")
            ...
            # ВАРИАНТ 1: Используем сохраненную ссылку
            if village_name in self.village_links:
                ...
                if article_id not in self.processed_article_ids:
                    ...
                    logger.info(f"        Поиск по сохраненной ссылке для {village_name} (ID: {article_id})")
            ...
            # ВАРИАНТ 2: Точный поиск с кавычками и типом (ОДИН ЗАПРОС!)
            ...
            logger.info(f"        Поиск координат для {village_name} (точный запрос)")
            ...
            # ВАРИАНТ 3: Поиск по названию и району (без кавычек) - ТОЛЬКО ЕСЛИ НАДО
            if type_short:
                simple_query = f"{village_name} {type_short} {district} район"
            else:
                simple_query = f"{village_name} {district} район"
            
            logger.info(f"        Поиск координат для {village_name} (упрощенный запрос)")
            results = await self._search_with_pagination(simple_query, max_pages=2, unlimited=False)
    ...
    async def fetch_district_data(self, district: str) -> List[Dict]:
        ...
        for settlement in settlements:
            try:
                ...
                # Страница с бывшими НП (уже может содержать координаты)
        ...
        # Шаг 5: Поиск координат для записей без них (ТОЛЬКО ЗДЕСЬ!)
        if all_villages:
            ...
            if villages_without_coords:
                logger.info(f"    • Сохраненных ссылок в кэше: {len(self.village_links)}")
                logger.info(f"    • Сохраненных координат в кэше: {len(self.coordinates_cache)}")
                ...
                for i, village in enumerate(sorted_villages):
                    try:
                        ...
                        # Пауза каждые 10 запросов (увеличено с 5 до 10 для скорости)
                        if i > 0 and i % 10 == 0:
                            await asyncio.sleep(1.5)
                ...
                # Исправленный подсчет остатка
                self.coords_stats['remaining'] = total_without - self.coords_stats['from_links'] - search_found - self.coords_stats['from_cache']
                ...
                logger.info(f"      • Всего найдено: {self.coords_stats['from_cache'] + self.coords_stats['from_links'] + search_found}")
