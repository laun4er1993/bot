...
# Версия с улучшенным поиском ссылок на НП и прямым парсингом координат
...
class APISourceManager:
    ...
    С улучшенным поиском ссылок на НП и прямым парсингом координат
    """
    ...
    async def _parse_settlement_main_page(self, article_id: str, district: str, settlement: str) -> List[Dict]:
        ...
        Сохраняет ссылки на отдельные страницы НП (УЛУЧШЕННАЯ ВЕРСИЯ)
        """
        url = DIC_ACADEMIC_ARTICLE_URL.format(article_id)
        html = await self._fetch_page(url)
    ...
    def _parse_settlements_section(self, html: str, article_id: str, district: str, settlement: str) -> List[Dict]:
        ...
        try:
            ...
            for table in unique_tables:
                ...
                # ЕСЛИ НЕ НАШЛИ ССЫЛОК ЧЕРЕЗ ИНДЕКСЫ, ИЩЕМ ПО ВСЕЙ ТАБЛИЦЕ
                if links_found == 0:
                    logger.info(f"        Не найдено ссылок по индексам, ищем по всей таблице")
                    for row in rows:
                        cells = row.find_all('td')
                        for i, cell in enumerate(cells):
                            ...
                            if link:
                                ...
                                # Находим тип в предыдущей ячейке
                                type_text = 'деревня'
                                if i > 0:
                                    prev_cell = cells[i-1]
    ...
    async def fetch_district_data(self, district: str) -> List[Dict]:
        ...
        С улучшенным поиском ссылок и прямым парсингом координат
        """
        self.clear_cache()
        ...
        if all_villages:
            ...
            if villages_without_coords:
                ...
                logger.info(f"    • Приоритетных записей: {len(priority_with_links) + len(priority_without_links)}")
