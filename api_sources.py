...
# Версия с улучшенным парсингом координат и увеличенным таймаутом
...
class APISourceManager:
    ...
    С улучшенным парсингом координат и увеличенным таймаутом
    """
    ...
    async def _parse_individual_village_page(self, article_id: str, district: str) -> Optional[Dict]:
        ...
        УЛУЧШЕННАЯ ВЕРСИЯ с поддержкой всех форматов
        """
        url = DIC_ACADEMIC_ARTICLE_URL.format(article_id)
        html = await self._fetch_page(url)
    ...
    def _parse_individual_village_html(self, html: str, article_id: str, district: str) -> Optional[Dict]:
        ...
        Ищет координаты в различных форматах
        """
        try:
            soup = BeautifulSoup(html, 'html.parser')
            ...
            # СПОСОБ 2: Ищем ссылку на geohack
            if not lat or not lon:
                geohack_link = soup.find('a', href=re.compile(r'toolserver\.org/~geohack'))
                if geohack_link:
                    href = geohack_link.get('href', '')
                    # Парсим params=56.310277787778_N_34.34250001_E
                    match = re.search(r'params=([\d.]+)_N_([\d.]+)_E', href)
                    if match:
                        try:
                            lat = float(match.group(1))
                            lon = float(match.group(2))
                            source = "geohack ссылка"
                        except:
                            pass
            
            # СПОСОБ 3: Ищем ссылку на Яндекс.Карты или Google Maps
            if not lat or not lon:
                yandex_link = soup.find('a', href=re.compile(r'maps\.yandex\.ru'))
                if yandex_link:
                    href = yandex_link.get('href', '')
                    # Формат: ?ll=34.34250001,56.310277787778
            ...
            # СПОСОБ 4: Ищем DMS формат (56°18′37″ с. ш. 34°20′33″ в. д.)
            if not lat or not lon:
            ...
            # СПОСОБ 5: Ищем десятичные координаты в тексте
            if not lat or not lon:
            ...
            if lat and lon:
                logger.info(f"        ✅ Найдены координаты для {name}: {round(lat,5)}, {round(lon,5)} (из {source})")
                return {
                    "name": name,
                    "type": village_type,
                    "lat": str(round(lat, 5)),
    ...
    async def _find_coordinates_for_village(self, village: Dict, district: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.debug(f"Ошибка поиска координат для {village['name']}: {e}")
            return None
    ...
    async def fetch_district_data(self, district: str) -> List[Dict]:
        ...
        for settlement in settlements:
            try:
                elapsed = time.time() - self.start_time
                if elapsed > 950:  # Увеличено для запаса
        ...
        if all_villages:
            ...
            if villages_without_coords:
                ...
                for i, village in enumerate(sorted_villages):
                    try:
                        elapsed = time.time() - self.start_time
                        if elapsed > 980:  # Увеличено до 980 секунд (оставляем запас 20 секунд)
