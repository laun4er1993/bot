...
# Версия с улучшенным поиском ссылок на НП и увеличенным таймаутом
...
class APISourceManager:
    ...
    С улучшенным поиском ссылок на НП и увеличенным таймаутом
    """
    ...
    def _parse_individual_village_html(self, html: str, article_id: str, district: str) -> Optional[Dict]:
        ...
        try:
            ...
            if not lat or not lon:
                ...
                if geo_dms:
                    # Пробуем найти координаты внутри
            ...
            if not lat or not lon:
                ...
                for elem in soup.find_all(['td', 'div', 'span']):
                    if 'Координаты' in elem.get_text():
                        # Ищем рядом ссылку с координатами
                        link = elem.find('a', href=re.compile(r'toolserver\.org'))
                        if link:
                            parent = link.find_parent()
                            if parent:
                                parent_text = parent.get_text()
                                # Пробуем найти DMS в тексте родителя
    ...
    async def _find_coordinates_for_village(self, village: Dict, district: str) -> Optional[Dict]:
        ...
        try:
            ...
            for result in results[:3]:  # Проверяем первые 3 результата
                # Проверяем наличие ключа score
                score = result.get('score', 0)
                if score > 50:  # Только релевантные
    ...
    async def fetch_district_data(self, district: str) -> List[Dict]:
        ...
        С прямым парсингом координат (без geohack)
        """
        ...
        for settlement in settlements:
            try:
                elapsed = time.time() - self.start_time
                if elapsed > 1350:  # Увеличено до 22.5 минут
        ...
        if all_villages:
            ...
            if villages_without_coords:
                ...
                for i, village in enumerate(sorted_villages):
                    try:
                        elapsed = time.time() - self.start_time
                        if elapsed > 1380:  # Увеличено до 23 минут
