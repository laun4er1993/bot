...
# Версия с проверкой кликабельности ссылок на НП
...
class APISourceManager:
    ...
    С проверкой кликабельности ссылок на НП
    """
    ...
    async def _parse_settlement_main_page(self, article_id: str, district: str, settlement: str) -> List[Dict]:
        ...
        if results:
            ...
            links_count = sum(1 for v in results if v.get('article_id'))
            if links_count > 0:
                logger.info(f"        Из них кликабельных: {links_count}")
        else:
            ...
            if alt_results:
                ...
                links_count = sum(1 for v in alt_results if v.get('article_id'))
                if links_count > 0:
                    logger.info(f"        Из них кликабельных: {links_count}")
                results = alt_results
                ...
                for row in rows[1:]:
                    try:
                        ...
                        results.append({
                            ...
                            "district": district,
                            "has_coords": False,
                            "article_id": article_id_from_link  # Сохраняем ID для последующего использования
                        })
    ...
    async def fetch_district_data(self, district: str) -> List[Dict]:
        ...
        if all_villages:
            ...
            if villages_without_coords:
                ...
                # Сначала обрабатываем те, у кого есть ссылки
